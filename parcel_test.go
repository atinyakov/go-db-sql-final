...
func TestAddGetDelete(t *testing.T) {
	...
	storedParcelId, err := store.Add(parcel)

	require.NoError(t, err)
	require.NotEmpty(t, storedParcelId)

	// get
	// получите только что добавленную посылку, убедитесь в отсутствии ошибки
	// проверьте, что значения всех полей в полученном объекте совпадают со значениями полей в переменной parcel
	p, err := store.Get(storedParcelId)

	require.NoError(t, err)
	require.Equal(t, storedParcelId, p.Number)
	...
	err = store.Delete(storedParcelId)
	require.NoError(t, err)
...
func TestSetAddress(t *testing.T) {
	...
	err = store.SetAddress(storedParcelId, newAddress)
	require.NoError(t, err)
...
func TestSetStatus(t *testing.T) {
	...
	err = store.SetStatus(storedParcelId, ParcelStatusSent)
	require.NoError(t, err)
